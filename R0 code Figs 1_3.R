lapply(c("ggplot2","ggthemes","deSolve","dplyr"),require,character.only=T) #load multiple packages
...
#Adjusting contact tracing efficiency w/ infections, contact tracers, contacts; only ratio important
...
ggplot(dat, aes(y=R0v, x=cases_per_tracer_calls,col=R0v)) + 
  ...
  ylab(expression(R[0]))+
  annotate(geom="text",x=1., y=ylh,#c(1.5, 1.7, 1.93, 2.08),
...
#Not in paper: Plot epsilon w/ contacts per tracer calls as X-axis - all data on one line/curve
...
ggplot(dat2, aes(y=R0v, x=cases_per_tracer_calls,col=R0v)) + 
  theme_bw()+geom_point(size=2)+scale_x_continuous(trans='log10')+
  coord_cartesian(xlim=c(.005, 50))+#, ylim = c(0.5,3))+
  scale_color_gradientn(colours=c("green","yellow","orange","red"))+
  theme(axis.title=element_text(size=25),legend.position = c(.8, .50),
        axis.text=element_text(size=25),legend.text=element_text(size=15),
        legend.title=element_text(size=15))+labs(colour = expression(R[0]))+
  geom_hline(yintercept=1)+
  #  xlab(expression(Log[10]~(Cases~per~Contact~Tracer~Calls~Per~Day)))+
  xlab(expression(Cases~per~Contact~Tracer~Calls~Per~Day))+
  ylab(expression(R[0]))+
...
par(mar=c(3, 3, .5, 0.5),mgp=c(1.5,.5,0),cex=2)
barplot(Rt_sens[,1],horiz=T,names.arg=parlabs,las=2,col="blue",xlim=c(-.11,.11))
