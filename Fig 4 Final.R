...
text(nt*.9,ht*.85,paste("Frac. est. ",sum(mxI>3*E[1])/NSIM))
...
mxI=apply(Ens, 2, function(x) max(x, na.rm = TRUE))
text(nt*.9,ht*.85,paste("Frac. est. ",sum(mxI>E[1])/NSIM))
...
mtext("Number of Latently Infected People", side = 2, outer = TRUE, line = 1.7,cex=1.7)
R0t[1]
dev.off()
