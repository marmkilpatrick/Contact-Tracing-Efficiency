...
#Ninf=100 # # of cases/infections traced
...
#tdelay=5;testdelay=rep(tdelay,each=length(Ninf))
...
#ggsave("Fig1.pdf",plot=p1,width=12,height=8)
ggsave("Fig1.tiff",plot=p1,width=12,height=8,units="in",compression = "lzw")
...
pars2 = data.frame(N=100001,S=100001,
                   ...
                   tau_Is=1/testdelay,
                   ftraced=capcontratio,
                   tracetime=0.5, #fixing avg time to trace to 1/2 day
                   CTR=T,
                   Ncont_p_inf= 10, # contacts per infection
                   NCCTD=12, # # of contacts a contact tracer can contact per day to isolate; 1.5/hr
                   NCT=15 # # of contact tracers for population
...
#ggsave("Fig2.pdf",plot=p2,width=12,height=8)
ggsave("Fig2.tiff",plot=p2,width=12,height=8,units="in",compression = "lzw")
...
#pdf("S3_Fig.pdf",width=10,height=8)
tiff("S3_Fig.tif",width=10,height=8,units="in",res=300,compression = "lzw")
...
barplot(Rt_sensR[,1],horiz=T,names.arg=parlabs,las=2,col="blue",xlim=c(-.11,.11),#xlab="Change in R0")
        xlab=expression(Change~"in"~"reproductive number,"~R[t]),las=1)
barplot(Rt_sensR[,2],horiz=T,names.arg=parlabs,las=2,col="red",add=T,las=1)
