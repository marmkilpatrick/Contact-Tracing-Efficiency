...
options(scipen=5);titlocH=240;titlocV1=99200;titlocV2=93200
...
dynSD=as.data.frame(ode(yini, times, SIRb, parsSD));dynSD$I=dynSD$Ips+dynSD$Is+dynSD$E
cl1=c(3,2,5);ctp=c("S","I","R") #cl1=c(3,1,2,7,4,5);
par(mfcol=c(2,3),mar=c(0, 3, 0, 0),mgp=c(1.5,.5,0),oma = c(.5, .5, 0.5, 0.5),cex=1.25)
matplot(x=dynSD[,1],y=dynSD[,ctp],type="l",xaxt="n",lty=1,lwd=2,ylim=c(0,103000),
...
legend(5,.8e5,c("S",expression(E+I[s]+I[ps]),"R"),col=cl1,lty=1,bty="n",seg.len=1,lwd=2,x.intersp=.5)
text(0,95000,"A")
...
plot(R0sd~dynSD[,1],xlab="",ylab=expression("Reproductive number, "~R[t]),type="l",ylim=c(0,2.0),col=8,lwd=2) #Rt over time
text(0,2.0,"D")

parsUCT= pars3 %>% mutate (kappa=1,CTR=T,NCT=1500)
dynUCT=as.data.frame(ode(yini, times, SIRb, parsUCT));dynUCT$I=dynUCT$Ips+dynUCT$Is+dynUCT$E
par(mar=c(0, 0, 0, 0),mgp=c(1.5,.5,0),cex=1.25)
matplot(x=dynUCT[,1],y=dynSD[,ctp],col=cl1,type="l",xaxt="n",yaxt="n",lty=1,lwd=2,ylim=c(0,103000),
...
dynLCT=as.data.frame(ode(yini, times, SIRb, parsLCT));dynLCT$I=dynLCT$Ips+dynLCT$Is+dynLCT$E
par(mar=c(0, 0, 0, 0),mgp=c(1.5,.5,0),cex=1.25)
matplot(x=dynLCT[,1],y=dynLCT[,ctp],col=cl1,type="l",xaxt="n",yaxt="n",lty=1,lwd=2,ylim=c(0,103000),
        xlab="Day",ylab="Number of people")
text(0,95000,"C")
text(titlocH,titlocV1,"No social distancing")
